...
class SimConverter:
    def __init__(self):
        ...
        self.pub_servo_angles = rospy.Publisher('/stewart_platform/position_cmd', Float32MultiArray, queue_size=1, tcp_nodelay=True) #servo angle publisher
        self.sub_platform_state = rospy.Subscriber('/' + robot_name + '/servo_setpoint/positions', ServoAngles6DoFStamped, self.callback, tcp_nodelay=True) #target pose subscriber
